...
public class Term implements VariableContainer {
	...
	public boolean isConstant() {
		return arity() == 0 && Character.isLowerCase(function.charAt(0));
	}

	public boolean isVariable() {
		return arity() == 0 && Character.isUpperCase(function.charAt(0));
	}
	...
	public void substitute(Term term, Term forVar) {
		if (isVariable() && this.equals(forVar)) {
	...
	/**
	 * Conversion depuis la structure de Term imaginée par Dominic Scheurer vers
	 * la structure de Terme imaginée par Vincent Iampietro.
	 * 
	 * @return Retourne un objet de type statique Terme et de type dynamique
	 *         Variable ou Fonction selon les proprietes de l'objet courant.
	 */
	public Terme toVincentTerm() {

		if (isConstant())
			return new Fonction(function);
		else if (isVariable())
			return new Variable(function);

		/*
		 * Construction d'un objet Fonction avec ses arguments
		 */
		ArrayList<Terme> args = new ArrayList<>();
		for (Term t : innerTerms)
			args.add(t.toVincentTerm());

		return new Fonction(function, args);
	}
}
